...
**`state.go`** — persistence. `State` (containing `[]Snag`) is marshalled to `.snags/state.yaml`. `LoadState` resets any `inflight` snags to `pending` on startup (crash recovery). `EnsureSnagDir` also appends `.snags/` to the repo's `.git/info/exclude`.
